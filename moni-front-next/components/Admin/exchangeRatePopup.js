...
const getExchangeRates = async () => {
	...
	const res = await Axios.get(`${process.env.NEXT_PUBLIC_MONI_API}/api/exchange`, header);
	return res.data;
};
...
const EchangeRatePopup = props => {
	...
	const handleSubmit = async e => {
		e.preventDefault();
		console.log({ id, compra, venta })
		const res = await putExchange(id, parseFloat(compra), parseFloat(venta));
