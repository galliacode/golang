...
const getCompra = async () => {
	const res = await Axios.get(
		`${process.env.NEXT_PUBLIC_MONI_API}/api/exchange/calculate?originCurrency=USD&destinationCurrency=PEN&amount=1`
	);
	return res.data;
};
...
const getVenta = async () => {
	const res = await Axios.get(
		`${process.env.NEXT_PUBLIC_MONI_API}/api/exchange/calculate?originCurrency=PEN&destinationCurrency=USD&amount=1`
	);
	return res.data;
};
		...
		try {
			...
			if (getAgent.length !== 0) {
				...
				for (let index = 0; index < getAgent.length; index++) {
					...
					const agentPro = await getAgentData(token, getAgent[index].id);
					// console.log(agentPro);
					setAgentProfile(agentPro);
					const agentAcc = await getAgentAccounts(token, getAgent[index].id);
