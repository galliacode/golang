...
const resetCommission = async (token, id) => {
	...
	const resAgent = await Axios.get(
		`${process.env.NEXT_PUBLIC_MONI_API}/api/agent/reset/` + id,
		header
	);
...
const getInvoice = async token => {
	...
	const resAgent = await Axios.get(
		`${process.env.NEXT_PUBLIC_MONI_API}/api/agent/invoice`,
		header
	);
	return resAgent.data;
};
														...
														<td>
															<button
																onClick={async e => {
																	...
																	setSelectedAgentId(operation.id);
